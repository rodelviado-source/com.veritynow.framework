        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                if (params && params.success) {
            	 ...
            	 console.log ("Failure to send the document...");
                if (params && params.failure) {
                    params.failure(event);
                } else {
                    console.log(event.target.response);
                    alert("Failure to send the document...");
                }
            }
        }
    }
...
function xhandler(url, submitType) {
	
	const ep = this.app.getEcmaParser()
	const blob = ep.getPDFBlob(); 
	console.log(blob, url, submitType);
	
	if (blob != null) {
	const xhr = new XMLHttpRequest();
    if (xhr.upload) {
    	console.log("sending to " + url + " as " + submitType);
        setRequestEventHandlers(xhr);
        xhr.open('POST', url, true);
        const fileName = document.querySelector("#FDFXFA_PDFName").textContent;
        if (submitType === "raw") {
            xhr.setRequestHeader("Content-Disposition", `filename="${fileName}"`)
