...
import { getAgent } from "@/features/auth/agent";
...
import { RecordDTO, Statuses, StatusValues } from "@/features/record/index";
...
export  function NewRecord() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const rec = await DataFacade.records.create(form);
      // upload images if provided
      if (files.length) {
        const response = await DataFacade.assets.upload(rec.id, files);
      }
      return rec;
    },
    onSuccess: () => {
          ...
          <div className="bg-white rounded-2xl p-6 w-[95%] max-w-3xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <select className="border rounded-2xl px-2 py-1" value={form.status} onChange={e => setForm({ ...form, status: e.target.value as any })}>
                  {StatusValues.map(s => (
                    <option key={s} value={s}>{s}</option>
                  ))}
                </select>
                <div className="text-sm text-gray-600 flex items-center">Uploaded: {form.uploaded ? 'Yes' : 'No'}</div>
              </div>
