...
import { TransportTypes } from "../transports/Transport";
...
export function loadRecs(): RecordItem[] {
  try { return JSON.parse(localStorage.getItem(KEYS.RECS) || "[]") as RecordItem[]; } catch { return []; }
}
export function saveRecs(v: RecordItem[]) { localStorage.setItem(KEYS.RECS, JSON.stringify(v)); }

export function loadImgs(): Record<string, string> {
  try { return JSON.parse(localStorage.getItem(KEYS.IMGS) || "{}") as Record<string, string>; } catch { return {}; }
}
export function saveImgs(v: Record<string, string>) { localStorage.setItem(KEYS.IMGS, JSON.stringify(v)); }
