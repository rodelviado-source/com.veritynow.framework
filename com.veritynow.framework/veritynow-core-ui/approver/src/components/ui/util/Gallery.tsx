...
const revokeIfBlob = (u: string | null | undefined) => {
...
function pickVersion(media: MediaResource | null | undefined): string {
  if (!media) return "";
...
function formatMediaMeta(media: MediaResource | null | undefined): string | null {
...
export default function Gallery({
  ...
  const prevThumbsRef = React.useRef<string[]>([]);
  React.useEffect(() => {
    prevThumbsRef.current.forEach(revokeIfBlob);
    ...
    (async () => {
      ...
      await Promise.all(
        imageIds.map(async (id, idx) => {
          try {
            const media = await DataFacade.mediaFor(id);
            if (cancelled) return;
            ...
            if (cancelled) return;
            prevThumbsRef.current.push(thumbUrl);
 ...
 useEffect(() => {
  return () => { prevThumbsRef.current.forEach(revokeIfBlob); prevThumbsRef.current = []; };
}, []);
          ...
          >
            ...
            >
              {current?.media && current.media.kind === MediaKind.pdf ? (
                 <PDFViewer key={viewerKey} {...current.media} />
              ) : loading ? (
                <CircularProgress />
              ) : (
