...
public final class MistralOutputFlattener {
    ...
    public static Map<String, OcrField> flattenToOcrFields(JsonNode extracted) {
        ...
        if (extracted == null || extracted.isNull()) return out;
        ...
        if (values != null && values.isObject()) {
            ...
            while (it.hasNext()) {
                ...
                out.put(k, new OcrField(k, v == null || v.isNull() ? "" : v.asText(), 1.0));
            }
        }

        JsonNode choices = extracted.get("choices");
        if (choices != null && choices.isObject()) {
            Iterator<Map.Entry<String, JsonNode>> it = choices.fields();
            while (it.hasNext()) {
                Map.Entry<String, JsonNode> e = it.next();
                String k = e.getKey();
